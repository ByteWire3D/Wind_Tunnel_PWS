...
void setup() {
  Serial.begin(115200);
  Wire.begin();
  motor.attach(motorPin, 900, 2100);
  //esc_calibration();
  motor.write(0);  // Initialize ESC to 0 throttle (usually 0 degrees)
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (next == 1) {
      float setpoint = 3.5;  // setpoint in m/s
...
void esc_calibration() {
  motor.write(180);  // set to 2000us puls length at boot up, tells max position to the esc
  while (!Serial) { delay(10); }
  Serial.println("max positon");
  delay(1000);

  motor.write(0);  //set to 1000us puls length, tells low position to esc
...
float pid(float airspeed, float setpoint) {
  ...
  output = Pout + Iout + Dout + offset;
  output_offset = better_contrain(output, 0, 180);
