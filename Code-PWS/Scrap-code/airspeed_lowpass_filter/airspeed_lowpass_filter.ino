...
float zeroAirspeedPressure = 7654.71;  // Raw pressure at zero airspeed
float slope = 1.0;
const float filterFactor = 0.15;  // Filter factor for smoothing pressure readings
...
float calc_airspeed() {
  ...
  // Apply filter to smooth pressure readings
  static float filteredPressure = 0.0;
  filteredPressure = (1 - filterFactor) * filteredPressure + filterFactor * rawPressure; // EMA filter to reduce noise

  // Calculate corrected differential pressure and make it absolute( no negitive values)
  float correctedPressure = abs(slope * (filteredPressure - zeroAirspeedPressure));
