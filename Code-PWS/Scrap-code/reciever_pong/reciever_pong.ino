...
void deserializeTestData(const uint8_t *buffer, TestData &data) {
  memcpy(&data, buffer, sizeof(TestData));
}

bool receiveTestData(HardwareSerial &serial, TestData &data) {
  if (serial.available() >= sizeof(TestData)) {
...
void waitForData(HardwareSerial &serial, TestData &data, unsigned long max_wait_time_ms) {
  unsigned long start_time = millis();
...
void setup() {
  ...
  waitForData(sd_card, data, 2000);  // Wait for data with a maximum wait time of ~47.6ms
}
